 ...
 */
export const wrapGaxiosPromise = async <T>(
...
describe('search', () => {
  ...
  beforeAll(async () => {
    ...
    await wrapGaxiosPromise(
      createTempFhirStore(
        `projects/${process.env.GCP_PROJECT}/locations/${process.env.GCP_REGION}/datasets/${process.env.TEST_FHIR_DATASET}`,
        process.env.TEST_FHIR_STORE_ID ?? '',
      ),
    );
    ...
    console.info(
      `bundle lengths: ${bundles.map(b => b.entry?.length).join(', ')}`,
    );
    await Promise.all(
      bundles.map((bundle: R4.IBundle) =>
        wrapGaxiosPromise(
          healthcare.projects.locations.datasets.fhirStores.fhir.executeBundle({
            parent,
            requestBody: bundle,
          } as healthcare_v1.Params$Resource$Projects$Locations$Datasets$Fhirstores$Fhir$Executebundle),
        ),
      ),
    );
  ...
  afterAll(async () => {
    ...
    await wrapGaxiosPromise(
      healthcare.projects.locations.datasets.fhirStores.delete({name: parent}),
    );
  ...
  it('should be able to find all active patients', async () => {
    ...
    console.info(bundle.entry?.length);
    expect(bundle.entry?.length).toBe(
      testPatients.length - testPatients.length / 50,
    );
  });
  it('should be able to find all deceased patients', async () => {
    const bundle = await fhirSearch(
      `${parent}/fhir/Patient`,
      [['death-date:missing', 'false']],
      defaultOpts,
    );
    console.info(bundle.entry?.length);
