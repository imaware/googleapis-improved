...
import {healthcareApiBaseUrl} from '..';
 ...
 */
const fhirSearch = async (
  resourceTypeUri: string,
  fhirSearchParams: [string, string][],
  opts: FhirSearchOpts = {maxPages: -1, maxPerPage: 1000, strict: false},
): Promise<R4.IBundle> => {
  console.debug(
    `calling fhirSearch: ${resourceTypeUri} - ${fhirSearchParams} - ${JSON.stringify(
      opts,
    )}`,
  );
  const {maxPages, maxPerPage, strict, link} = opts;
  // Build the search parameters
  const params = new URLSearchParams(fhirSearchParams);
  // Pagination
  params.set('_count', maxPerPage.toString());
  const token = await google.auth.getAccessToken();
  // Perform search either with a link or with the search parameters
  const res = await axios.get<R4.IBundle>(
    link ? link : `${healthcareApiBaseUrl}/${resourceTypeUri}`,
    {
      headers: {
  ...
  if (E.isRight(validationResult)) {
    ...
    // Check for next page
    const nextPage = bundle.link?.find(
      (link: R4.IBundle_Link) => link.relation === 'next',
    );
    // If we have a next page, recursively call this function
    if (nextPage?.url && maxPages !== 0) {
      const nextBundle = await fhirSearch(resourceTypeUri, fhirSearchParams, {
        maxPages: maxPages - 1,
        maxPerPage,
        strict,
        link: nextPage.url,
